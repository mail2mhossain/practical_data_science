...
#X_train = np.load('vgg16_dogs_vs_cats_train_features.npy')
#y_train = np.load('vgg16_dogs_vs_cats_train_labels.npy')
data = np.load('vgg16_dogs_vs_cats_train_data.npz')
X_train, y_train = data['arr_0'], data['arr_1']

#X_test = np.load('vgg16_dogs_vs_cats_test_features.npy')
#y_test = np.load('vgg16_dogs_vs_cats_test_labels.npy')
data = np.load('vgg16_dogs_vs_cats_test_data.npz')
...
# save model
f = open('vgg16_xgb_best_model', "wb")
f.write(pickle.dumps(clf_xgb))
f.close()

#load model
model = pickle.loads(open('vgg16_xgb_best_model', "rb").read())
predictions = model.predict(X_test)
print(classification_report(y_test, predictions))
plot_confusion_matrix(model, X_test, y_test, values_format='d', display_labels=["Dog","Cat"])
pyplot.savefig("vgg16_confusion_matrix_dogs_cats_plot.png")
# show the figure
pyplot.show()